...
for (let i = 0; i < numbers.length; i++) {
  numbers[i].addEventListener("click", (e) => {
    if (
      !isFirstNumber &&
      displayNumber.textContent.length <= 9 &&
      ...
      displayNumber.textContent.length <= 9 &&
      displayNumber.textContent.includes(".") ||
      displayNumber.textContent.length == 10 &&
      displayNumber.textContent.includes(".")
    ) {
      getSecondValue(e);
    } else if (isFirstNumber && displayNumber.textContent.length <= 8 || isFirstNumber && displayNumber.textContent.length == 9) {
      getSecondValue(e);
    }
  });
}
...
commaButton.addEventListener("click", () => {
  if (
    ...
    isFirstNumber &&
    !displayNumber.textContent.includes(".") &&
    !secondNumber.toString().length == 0
  ) {
    secondNumber += ".";
    displayNumber.textContent = secondNumber;
  }
...
function divide(a, b) {
    b == 0 ? displayNumber.textContent = "ERROR" : returnValue = a / b;
    if (returnValue.toString().length > 10) {
      returnValue = +returnValue.toString().slice(0, 10);
    };
};